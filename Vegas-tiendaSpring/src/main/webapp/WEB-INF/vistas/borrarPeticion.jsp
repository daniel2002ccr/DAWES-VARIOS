...
<body>
	...
	<div class="container">
		...
		<div class="form">
			<form
				action="http://localhost:8080/tienda/peticiones/formularioborrarpeticion"
				method="POST">
...
<c:forEach items="${lista}" var="peticion">
    <div class="form">
        <form action="http://localhost:8080/tienda/peticiones/borrarpeticion" method="POST">

            <label for="id">ID Peticion</label>
            <input type="text" id="id" name="id" value="${peticion.id}" readonly>

            <label for="cliente">Cliente</label>
            <select name="cliente" id="cliente" readonly>
                <c:forEach items="${combosCliente}" var="cliente">
                    <option value="${cliente.id}">${cliente.nombre}</option>
                </c:forEach>
            ...
            <label for="producto">Producto</label>
            <select name="producto" id="producto" readonly>
                <c:forEach items="${combosProducto}" var="producto">
                    <option value="${producto.id}">${producto.nombre}</option>
                </c:forEach>
            ...
            <label for="cantidad">Cantidad</label>
            <input type="text" id="cantidad" name="cantidad" value="${peticion.cantidad}" readonly>
            ...
            <input type="date" id="fecha" name="fecha" value="${peticion.fecha}" readonly>
...
<c:if test="${resultado == 1}">
    <h3>Peticion borrada con éxito</h3>
</c:if>
</body>
</html>